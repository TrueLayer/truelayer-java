...
class PaymentHandlerTests {
    ...
    public void itShouldYieldAPaymentWhenCreatingAPayment() {
        ...
        var response = paymentHandler.createPayment(
                CreatePaymentRequest.builder().amountInMinor(100).build());
    ...
    private PaymentHandler stubCreatePaymentResponse(CreatePaymentResponse paymentResponse) {
        ...
        var apiResponse = stubApiResponse(success(ApiResponse.<CreatePaymentResponse>builder()
                .data(paymentResponse)
                .build()));
        when(paymentsApi.createPayment(any(CreatePaymentRequest.class))).thenReturn(apiResponse);
        return PaymentHandler.builder().paymentsApi(paymentsApi).build();
    }

    private PaymentHandler stubGetPaymentByIdResponse(BasePaymentDetail paymentResponse) {
        var paymentsApi = Mockito.mock(IPaymentsApi.class);
        var apiResponse = stubApiResponse(success(
                ApiResponse.<BasePaymentDetail>builder().data(paymentResponse).build()));
        when(paymentsApi.getPayment(anyString())).thenReturn(apiResponse);
        return PaymentHandler.builder().paymentsApi(paymentsApi).build();
    }
}
